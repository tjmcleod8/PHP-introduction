<html>
    ...
    <body class="php">
        <?php
            ...
            // *********************************************Create SQL query
            $query = "insert into equipment (serial_number, cmp_name, owner, rm, asset_type, " . 
                "allocation, make, model, purchase_date, repl, repl_yr, amount)" .
                "values (?,?,?,?,?,?,?,?,?,?,?,?)";
            $stmt = $conn->prepare($query);
            $stmt->bind_param("sssssssssssd", $serialNumber, $cmpName, $Lname, $RmNum, $Asset, $Allocation,
                $Make, $Model, $PurchaseDate, $Repl, $ReplYr, $Amount);
